...
resource "aws_subnet" "public1" {
  ...
  tags = { Name = "sams-suit-shop-public-1" }
}
...
resource "aws_security_group" "alb" {
  ...
  tags = { Name = "sams-suit-shop-alb-sg" }
}

resource "aws_security_group" "ecs" {
  name   = "sams-suit-shop-ecs-sg"
  vpc_id = aws_vpc.main.id
  ...
  tags = { Name = "sams-suit-shop-ecs-sg" }
}
...
resource "aws_lb" "main" {
  ...
  tags = { Name = "sams-suit-shop-alb" }
}

# ALB Target Groups
resource "aws_lb_target_group" "backend" {
  name        = "sams-suit-shop-backend-tg"
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
  ...
  tags = { Name = "sams-suit-shop-backend-tg" }
}

resource "aws_lb_target_group" "frontend" {
  name        = "sams-suit-shop-frontend-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
  ...
  tags = { Name = "sams-suit-shop-frontend-tg" }
}
...
resource "aws_ecs_cluster" "main" {
  ...
  tags = { Name = "sams-suit-shop" }
}

# CloudWatch Log Group
resource "aws_cloudwatch_log_group" "ecs" {
  name              = "/ecs/sams-suit-shop"
  retention_in_days = 7

  tags = { Name = "sams-suit-shop-logs" }
}

# ECS Task Execution Role
resource "aws_iam_role" "ecs_task_execution_role" {
  name = "sams-suit-shop-ecs-task-execution-role"
...
# ECS Task Role for ECR access
resource "aws_iam_role" "ecs_task_role" {
  name = "sams-suit-shop-ecs-task-role"
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "backend"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/sams-suit-shop-backend:latest"
      essential = true
      portMappings = [
        {
          containerPort = 3000
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "frontend"
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/sams-suit-shop-frontend:latest"
      essential = true
      portMappings = [
        {
          containerPort = 80
